  ...
  for (i in (1 - burn.in):(ndraw * thinning)) {
    ...
    if (i > 0) {
      ...
      if (computeMarginalEffects) {
        ...
        probz <- pnorm(as.numeric(mu) / sige)     # Phi(Xb/sigma) = Prob(z <= Xb/sigma)
...
else if (is.null(c)) {    # case of informative prior rho ~ N(c,T)
 z <- I_n - rho * W
 e <- z%*% - xb
 n <- length(y)
 T <- T*sige
...
marginal.effects.sartobit <- function (object, o = 100, ...) {
    ...
    for (i in 1:ndraw) {
        ...
        rhovec <- rho^(0:(o - 1))
        probz <- pnorm(as.numeric(mu) / sigma)     # Phi(Xb/sigma) = Prob(z <= Xb/sigma)
